...
class Metrics():
    ...
    def score_precision_recall(self, y_true, y_pred):
        ...
        for sentence_counter, relations in enumerate(y_true["y"]):
            for relation in relations:
                true_relations.append((sentence_counter, relation))

        true_list = true_relations

        pred_relations = list()

        for sentence_counter, relations in enumerate(y_pred["y"]):
            for relation in relations:
                pred_relations.append((sentence_counter, relation))

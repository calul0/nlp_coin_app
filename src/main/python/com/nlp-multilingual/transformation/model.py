...
import pandas as pd
import pickle
from transformation.annotator import map_find_max_entity, find_max_entity
import pathlib
...
def load_ner_model_v2(directory, model_name, id_col="DesignID", design_col="Design", load_optional=False):
    return DesignEstimator(
        ...
        id_col,
        design_col,
        spacy_model=directory,
        load_optional=load_optional
    )


def predict_re_single_sentence(model: sklearn.pipeline.Pipeline,
                               sentence: str,
                               id_col: str,
                               design_col: str) -> List[Tuple]:
    """
    Function takes the model and return the prediction from a specific sentence.

    Args:
        model (sklearn.pipeline.Pipeline): Model, which was trained already
        sentence (str): The sentence to be predicted
        id_col (str): The column name of the id column
        design_col (str): The column name of the design column

    Returns:
        y_pred (List[Tuple]): Prediction result
    """
